...
establish_cluster_sizing_dynamic(working_folder)
...
for img_num in range(len(os.listdir(test_set_path))):
    ...
    if tracking_option:
        ...
        j=0
        for poly in polygons[-1]:
            if len(polygons_info[-1][j]) > 1:
                sorted_string.append([poly[0],polygons_info[-1][j][6][0]])
            else:
        ...
        #To remove any double tracked clusters
        #NEEDS IMPROVEMNT. CURRENTLY JUST REMOVES THE CLUSTER RECOGNIZED LATER
        polygons = block_overlapping_with_lower_confidence_polygons(polygons,iou_threshold=0.7)
    ...
    for poly in polygons[-1]:
        #Draw lines
        if len(polygons_info[-1][j]) > 1:
            print(j,polygons_info[-1][j][6][0])
        else:
            print(j,polygons_info[-1][j])
        if len(poly) > 1 and len(polygons_info[-1][j][6]) == 1:
            poly.reshape(-1,1,2)
            ...
            if cluster_sizing_option:
                ...
                write_cluster_sizing_dynamic(cluster_segments[-1],working_folder)
            ...
            #cluster_track[-1].append(j)
        #To draw clusters that are blocked
        elif len(poly) > 1:
            if visualise_harvest == True:
                poly.reshape(-1,1,2)
                #Getting the centre point of the polygons
                centre = Polygon(poly).centroid
    ...
    save_image(working_folder,full_image,sizing_image,cluster_sizing_option,img_num)
    if cluster_sizing_option:
    ...
    #To track clusters in the next image
    #cluster_track.append([])
